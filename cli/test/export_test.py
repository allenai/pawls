...
class TestExport(unittest.TestCase):
    ...
    def test_export_annotation_from_all_annotators(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            assert not os.path.exists(
                os.path.join(tempdir, 'annotations.json'))

    def test_export_annotation_from_multiple_annotators(self):
        runner = CliRunner()
        with tempfile.TemporaryDirectory() as tempdir:
            result = runner.invoke(export, [
                                   self.TEST_ANNO_DIR, self.TEST_CONFIG_FILE, tempdir, '-u', self.USERS[0], '-u', self.USERS[1]])
            assert result.exit_code == 0
    ...
    def test_export_annotation_from_single_annotator(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            result = runner.invoke(
                export, [self.TEST_ANNO_DIR, self.TEST_CONFIG_FILE, tempdir, '-u', self.USERS[0]])
            assert result.exit_code == 0
            ...
            all_img_shas = set(
                [pth.split('_')[0] for pth in os.listdir(os.path.join(tempdir, 'images'))])
    ...
    def test_export_annotation_with_all_annotators_annotations(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            result = runner.invoke(export, [
                                   self.TEST_ANNO_DIR, self.TEST_CONFIG_FILE, tempdir, '-u', self.USERS[0], '--include-unfinished'])
            assert result.exit_code == 0
    ...
    def test_export_annotation_with_missing_annotation_file(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            result = runner.invoke(export, [
                                   self.TEST_ANNO_DIR, self.TEST_CONFIG_FILE, tempdir, '-u', self.USERS[1], '--include-unfinished'])
            assert result.exit_code == 0
