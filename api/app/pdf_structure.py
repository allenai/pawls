...
def to_dict_patch(self):
    ...
    for attr, _ in six.iteritems(self.openapi_types):
        ...
        if isinstance(value, list):
            ...
            result[attr] = dict(map(handle_dict,
                value.items()
            ))
        else:
            result[attr] = value
...
def get_annotations(
    ...
    if token_sources:
        if full_annotations.tokens is None:
            raise HTTPException(status_code=404, detail=f"Non-existent token annotations for sha {sha}")
        ...
        if "all" in token_sources:
            ...
            for source in token_sources:
                ...
                if data is None:
                    raise HTTPException(status_code=404, detail=f"Non-existent token source f{source} for sha {sha}")

                response["tokens"]["sources"][source] = data.to_dict()

    if text_element_sources:
        if full_annotations.text_elements is None:
            raise HTTPException(status_code=404, detail=f"Non-existent text element annotations for sha {sha}")
        ...
        if "all" in text_element_sources:
            ...
            for source in text_element_sources:
                ...
                if data is None:
                    raise HTTPException(status_code=404, detail=f"Non-existent text element source f{source} for sha {sha}")

                response["text_elements"]["sources"][source] = data.to_dict()

    if region_sources:
        if full_annotations.regions is None:
            raise HTTPException(status_code=404, detail=f"Non-existent region annotations for sha {sha}")
        ...
        if "all" in region_sources:
            ...
            for source in region_sources:
                ...
                if data is None:
                    raise HTTPException(status_code=404, detail=f"Non-existent region source f{source} for sha {sha}")
